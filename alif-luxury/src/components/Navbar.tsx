...
export const Navbar = () => {
  ...
  const { toggleCart, toggleAuth, toggleSearch, cart } = useStore();
  ...
  const navLinks = [
    ...
    { name: 'Lookbooks', href: '/lookbooks', isRoute: true },
    { name: 'Track Order', href: '/track', isRoute: true },
  ];

  const handleLinkClick = (e: ReactMouseEvent<HTMLAnchorElement>, link: { href: string, isRoute?: boolean }) => {
    if (link.isRoute) {
      e.preventDefault();
    ...
    <>
      ...
      >
        <div className="flex items-center justify-between">
          ...
          <a
            href="/"
            onClick={(e) => handleLinkClick(e, { href: '/', isRoute: true })}
          ...
          <div className="hidden md:flex items-center space-x-4">
            <button onClick={toggleSearch} className="relative p-2 hover:bg-brand-text/5 rounded-full transition-colors group">
                <Search size={20} className={isScrolled ? 'text-brand-text group-hover:text-brand-accent' : 'text-brand-background group-hover:text-brand-accent'} />
            </button>
          ...
          <div className="md:hidden flex items-center space-x-2">
            <button onClick={toggleSearch} className="relative p-2 hover:bg-brand-text/5 rounded-full transition-colors group">
                <Search size={24} className={isScrolled ? 'text-brand-text group-hover:text-brand-accent' : 'text-brand-background group-hover:text-brand-accent'} />
            </button>
            <button onClick={toggleAuth} className="relative p-2 hover:bg-brand-text/5 rounded-full transition-colors group">
